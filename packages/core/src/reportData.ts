import type { InitOptions, ReportData } from '@null/monitor-types'
import { EventTypes, SDK_VERSION } from '@null/monitor-shared'
import { _support, generateUUID, getLocationHref, isBrowserEnv, isEmpty, logger, Queue, validateOption } from '@null/monitor-utils'
...
export class TransportData {
  ...
  // 用于在用户离开页面时发送分析数据或日志信息，因为它不会延迟页面的卸载或跳转过程
  ...
  async beforePost(data: Partial<ReportData>): Promise<ReportData | boolean> {
  ...
  getTransportData(data: any): ReportData {
    ...
    // 性能数据、白屏检测等不需要附带用户行为
    const excludeReadCrumb = [
      EventTypes.PERFORMANCE,
      EventTypes.WHITESCREEN,
    ]
    if (!excludeReadCrumb.includes(data.type)) {
  ...
  getAuthId(): string | number {
    if (typeof this.getUserId === 'function') {
      ...
      if (typeof id === 'string' || typeof id === 'number') {
        ...
        logger.error(`userId: ${id} 期望 string 或 number 类型，但是传入 ${typeof id}`)
      }
    }
    return ''
  }
  ...
  isSdkTransportUrl(targetUrl: string): boolean {
    let isSdkDsn = false
    if (this.errorDsn && targetUrl.includes(this.errorDsn)) {
      isSdkDsn = true
    }
    return isSdkDsn
  }
  ...
  // 上报数据
  async send(data: Partial<ReportData>) {
